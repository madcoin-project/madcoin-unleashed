...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Madcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Madcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeMadcoinamount=<n> " + _("Keep N Madcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Madcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Madcoin is probably already running."), strDataDir));
    ...
    nAnonymizeMadcoinAmount = GetArg("-anonymizeMadcoinamount", 0);
    if(nAnonymizeMadcoinAmount > 999999) nAnonymizeMadcoinAmount = 999999;
    if(nAnonymizeMadcoinAmount < 2) nAnonymizeMadcoinAmount = 2;
    ...
    LogPrintf("Anonymize Madcoin Amount %d\n", nAnonymizeMadcoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1MDC+1000 == (.1MDC+100)*10
       10MDC+10000 == (1MDC+1000)*10
    */
