		...
		for (unsigned int i = 0; i < tx.vin.size(); i++) {
			...
			if(GetTransaction(tx.vin[i].prevout.hash, txPrev, hashBlock)) { 
				...
				if (ExtractDestination(txPrev.vout[tx.vin[i].prevout.n].scriptPubKey, source)) {  
					CMadcoinAddress addressSource(source);
					std::string txAddrStr = addressSource.ToString();
					BOOST_FOREACH(const std::string addr, blockedAddresses) {	   
						if (txAddrStr == addr) {
							return error("Error: Banned Address %s tried to send a transaction (rejecting it).", addressSource.ToString().c_str());
						}
					}
				}
			}
...
string getDevAddress(int nHeight) {
	...
	if (addrId == 0) {
		return "MHg5X4miatnhhLsiVt834p4Ty5NoPuKRVd";
	} else if (addrId == 1) {
		return "MWMrbc1jrhZhHfMBamX8MJTmNhUHkE7Aav";
	} else if (addrId == 2) {
		return "MUcVAmKvbKgVbR5dFaZrXaMvqU5Y5s22zh";
	} else {
		return "MF2RNFmpEbUdNBexbdfh4U8XfZqGou3wGE";
	}
}
...
const int YEARLY_BLOCKCOUNT = 394200; 
...
int64_t GetProofOfStakeReward(int nHeight, const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	if (nHeight >= 277777) {	
     nSubsidy = 10 * COIN;
	 if (nHeight == 277777) { nSubsidy = 10000000 * COIN; } // SWAP AMOUNT
	 if (nHeight > 277777 && nHeight <= 300000) {	nSubsidy = 5   * COIN; }
	 if (nHeight > 300000 && nHeight <= 310000) {	nSubsidy = 50  * COIN; }
	 if (nHeight > 310000 && nHeight <= 320000) {	nSubsidy = 75  * COIN; }
	 if (nHeight > 320000 && nHeight <= 330000) {	nSubsidy = 80  * COIN; }
	 if (nHeight > 330000 && nHeight <= 350000) {	nSubsidy = 85  * COIN; }
	 if (nHeight > 350000 && nHeight <= 400000) {	nSubsidy = 100  * COIN; }
	 if (nHeight > 400000 && nHeight <= 460000) {	nSubsidy = 125  * COIN; }
	 if (nHeight > 460000 && nHeight <= 500000) {	nSubsidy = 100  * COIN; }
	 if (nHeight > 500000 && nHeight <= 560000) {	nSubsidy = 85  * COIN; }
	 if (nHeight > 560000 && nHeight <= 620000) {	nSubsidy = 75  * COIN; }
	 if (nHeight > 620000 && nHeight <= 640000) {	nSubsidy = 70  * COIN; }
	 if (nHeight > 640000 && nHeight <= 660000) {	nSubsidy = 65  * COIN; }
	 if (nHeight > 660000 && nHeight <= 700000) {	nSubsidy = 55  * COIN; }
	 if (nHeight > 700000 && nHeight <= 760000) {	nSubsidy = 35  * COIN; }
	 if (nHeight > 760000 && nHeight <= 850000) {	nSubsidy = 25  * COIN; }
	 if (nHeight > 850000) { nSubsidy = 20 * COIN; }	 
	}	 
    return nSubsidy + nFees;
}
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	if (pindexLast->nHeight >= 277776 && pindexLast->nHeight <= 277778) {
            ...
            if(IsProofOfStake() && pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    if (pindex->nHeight >= 280000) {
                        CMadcoinAddress devRewardAddress(getDevAddress(pindex->nHeight + 1));
                    ...
                    CMadcoinAddress address2(address1);
                    if (!foundDevFee) {
