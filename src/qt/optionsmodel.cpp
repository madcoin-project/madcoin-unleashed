...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeMadcoinAmount"))
        settings.setValue("nAnonymizeMadcoinAmount", 1000);
    nAnonymizeMadcoinAmount = settings.value("nAnonymizeMadcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeMadcoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeMadcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeMadcoinAmount:
            return QVariant(nAnonymizeMadcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeMadcoinAmount:
            nAnonymizeMadcoinAmount = value.toInt();
            settings.setValue("nAnonymizeMadcoinAmount", nAnonymizeMadcoinAmount);
            emit AnonymizeMadcoinAmountChanged(nAnonymizeMadcoinAmount);
            break;
        default:
            break;
        }
