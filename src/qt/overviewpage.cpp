...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeMadcoinAmount = BitcoinUnits::formatWithUnit(false, nAnonymizeMadcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeMadcoinAmount = strAnonymizeMadcoinAmount.remove(strAnonymizeMadcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMadcoinAmount + " MDC / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeMadcoinAmount*COIN) nMaxToAnonymize = nAnonymizeMadcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeMadcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeMadcoinAmount));
        strAnonymizeMadcoinAmount = strAnonymizeMadcoinAmount.remove(strAnonymizeMadcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeMadcoinAmount + " MDC / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
